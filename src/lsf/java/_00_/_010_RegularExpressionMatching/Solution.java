package lsf.java._00_._010_RegularExpressionMatching;
...
public class Solution {
            ...
            for (int j = 1; j <= p_len; j++) {
                ...
                if (p_char == '.' || s_char == p_char)
                    ...
                    if (flag[i][j - 1])
                        ...
                        flag[i][j] = true;
                    else if (p.charAt(j - 2) == '.' || p.charAt(j - 2) == s_char)
                        flag[i][j] = flag[i - 1][j];
                }
            }
        }
    ...
    @Test
    public void t7() {
        Assert.assertEquals(s.isMatch("aaa", "ab*a"), false);
    }
